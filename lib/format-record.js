...
module.exports = function formatRecord(rec, opts) {
  ...
  case OM_LONG:
    ...
    var srcPath = '';
    if (rec.src) {
      srcPath = rec.src.file;
      if (short) {
        var srcPathRegEx = new RegExp('^' + process.cwd());
        srcPath = srcPath.replace(srcPathRegEx, '.');
      }
      var srcStr = srcPath + ':' + rec.src.line;
      srcPath = stylize(' ' + srcStr, 'brightBlack')
    }
